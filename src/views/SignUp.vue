<template>
  <div class="content-block__auth">
    <div class="container">
      <h2 v-show="!formDone">Регистрация</h2>
      <transition enter-active-class="animate__animated animate__fadeIn"
        leave-active-class="animate__animated animate__fadeOut"
        appear appear-active-class="icon-appear">
      ...
      <form v-if="!formDone" @submit.prevent="tryLoginWithTimers(3500)">
        <ul class="flex-outer">
          ...
          <li>
            <button type="submit" :disabled="!formReady">Зарегистрироваться</button>
          </li>
        </ul>
      </form>
      <template v-else>
        <h2>Вы успешно зарегистрированы.</h2>
        <p>Вы будете перенаправлены на страницу логина для авторизации через 3 секунды... <fa-icon size="lg" icon="fas fa-spinner" class="fa-pulse"/></p>
      </template>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async tryLoginWithTimers(timout) {
      await this.tryLogin();
      if (this.formDone) {
        await new Promise(resolve => setTimeout(resolve, timout));
        location.reload();
        this.$router.push({ name: 'login-route' }).then(() => {
          });
      } else {
